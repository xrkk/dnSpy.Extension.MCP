#!/usr/bin/env python3
"""P06 round-1 packaging orchestrator (host side).

Deploys the P06 build (plugin DLL, harness publish, integration drivers, the
ImportHost fixture compiled with the VM's framework csc — the isolated VM has no
NuGet access), then for each architecture provides a fresh dnSpy + clean
ArtifactRoot (the pairing rule) and runs the ACC-005 final-judgment evidence and
the headless --import-matrix harness case."""
...
# VM fact (round 1): dnSpy v6.6 runs .NET Framework for BOTH exes, so both
# architectures load the net48 extension build.
PLUGIN = {
    "x64": ROOT / "dist/dnSpy.Extension.MCP-net48.x.dll",
    "x86": ROOT / "dist/dnSpy.Extension.MCP-net48.x.dll",
}
DRIVER_CASES = ("EDIT-ACC-005",)
HARNESS_CASES = ("EDIT-ACC-031",)
...
def upload(client: UiMcpClient, source: Path, destination: str) -> None:
    ...
    client.call_tool_json("FileSystem", {
        "mode": "write", "path": destination + ".b64",
...
def upload_tree(client: UiMcpClient, entries: list[tuple[Path, str]], destination_root: str) -> None:
    with tempfile.NamedTemporaryFile(prefix="p06-deploy-", suffix=".tar.gz", delete=False) as temp:
    ...
    try:
        ...
        upload(client, archive_path, destination_root + r"\p06-deploy.tar.gz")
    finally:
        archive_path.unlink(missing_ok=True)
    powershell(client, f'tar.exe -xzf "{destination_root}\\p06-deploy.tar.gz" -C "{destination_root}"; "extracted"')


def deploy_plugin(client: UiMcpClient, arch: str) -> str:
    dll = PLUGIN[arch]
    expected = hashlib.sha256(dll.read_bytes()).hexdigest().upper()
    staged = VM_EXTENSION + ".p06.new"
...
def deploy(client: UiMcpClient) -> None:
    ...
    with tempfile.NamedTemporaryFile(prefix="p06-harness-", suffix=".tar.gz", delete=False) as temp:
        archive_path = Path(temp.name)
    try:
        with tarfile.open(archive_path, "w:gz") as archive:
        ...
        upload(client, archive_path, HARNESS_DEST + r"\p06-harness.tar.gz")
    finally:
        archive_path.unlink(missing_ok=True)
    powershell(client, (
        f'New-Item -ItemType Directory -Force -Path "{HARNESS_DEST}" | Out-Null; '
        f'Get-ChildItem "{HARNESS_DEST}" -Exclude p06-harness.tar.gz | Remove-Item -Recurse -Force -ErrorAction SilentlyContinue; '
        f'tar.exe -xzf "{HARNESS_DEST}\\p06-harness.tar.gz" -C "{HARNESS_DEST}"; "harness ready"'))
    print("[deploy] ImportHost fixture via framework csc", flush=True)
    ...
    build = powershell(client, (
        ...
        f'if(-not (Test-Path "$out86\\ImportHost.exe")){{ throw "ImportHost x86 build failed" }}; '
        f'Get-ChildItem $out | Select-Object Name,Length | ConvertTo-Json -Compress'
    ), timeout=120)
    print(f"[deploy] ImportHost: {build.strip()[:220]}", flush=True)
...
def main() -> int:
    client = UiMcpClient(VM_URL, timeout=90, client_name="p06-round1-orchestrator")
    client.initialize()
    deploy(client)
    results: list[dict] = []
    deployed = set()
    for arch in ("x64", "x86"):
        run_id = f"p06-import-20260912-r1-{arch}"
        marker = PLUGIN[arch].name
        if marker not in deployed:
            print(f"[{arch}] deploying {marker}", flush=True)
            deploy_plugin(client, arch)
            deployed.add(marker)
