...
''')
    ...
    assert len(outcomes) == 2, run.stdout
    for outcome in outcomes:
        for resource_type in (('embedded', 'linked') if outcome['kind'] == 'managed_resource_add' else ('win32',)):
            sha = 'a' * 64
            ...
            errors = list(jsonschema.Draft202012Validator(contract['edit_resource_import']['outputSchema']['oneOf'][0]).iter_errors(envelope))
            assert not errors, '\n'.join(str(list(e.absolute_path)) + ': ' + e.message for e in errors)
            print('PASS complete resource-import envelope:', resource_type, outcome['kind'])
