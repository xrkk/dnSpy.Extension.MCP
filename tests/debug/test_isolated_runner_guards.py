...
ALLOWED = (
    'ACC-001', 'ACC-002', 'ACC-003', 'ACC-023',
...
class IsolatedRunnerGuards(unittest.TestCase):
    ...
    def test_isolated_parameters_are_fail_closed(self):
        ...
        self.assertIn('isolated private port is already listening', TEXT)
    ...
    def test_allowed_handlers_have_no_shared_or_global_writes(self):
        ...
        for case in ALLOWED:
            with self.subTest(case=case):
                ...
                body = bodies[case]
                if case == 'ACC-023':
                    # The in-process SafeDefaults assertion deliberately names the product's
                    # 15378 default; it is not a listener launched by this handler.
                    self.assertIn("defaults: server off, 192.168.204.149:15378", body)
                    body = re.sub(r"(?m)^\s*Assert-Cond 'a23-default-snapshot'.*$", '', body)
                unsafe = re.search(r'C:\\Tools|\bnetsh\b|\bStop-Process\b|Get-Process\s+dnSpy|15378|15379', body)
