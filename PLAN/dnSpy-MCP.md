...
dnSpy 本身的功能中有一些对程序集的 "编辑" 功能, 此 MCP 是否支持? 列出 dnSpy 本身功能和 MCP 支持功能的表格.

我计划在此 MCP 中添加对这些 dnSpy 导出功能的支持 (用新分支). 这只是模糊的想法, 你如果有不清晰的, 可以通过 grill-me 向我提问. (我未明确下达指令前, 不许写文档或改代码) (如果需要 grill-me, 一次只提一问, 提供选项 ABC, 给出你推荐的选项和原因 - 推荐的选项放 A. 另外, 明显你能自己决定的不许问)

P01: 公共契约、权威调用上下文、VM 执行门禁、调试基线

> 2026.09.03:

P02: 事务内核、私有副本、统一审查、常用结构化编辑

```
P03: 检查点、谱系、Undo/Redo、重放、导出、旧工具迁移
P04: 高级结构化元数据编辑
P05: dnSpy 公开 C# 编译前端及安全限制
P06: MCP 自有导入器、C# 方法/类型编辑、内嵌符号
P07: 程序集/模块身份、入口点、跨程序集影响分析
P08: 托管/Win32 资源、大载荷、最小强名称兼容
P09: dnSpy UI、文档、测试提示词和最终全量验收
```
