...
- 使用仓库 `tests/edit/run_p01_vm_tests.py` 作为总编排，并通过 Win10VM 管理 MCP 部署与运行；不得要求人工点击或人工切换 x86/x64。
- 只操作 `C:\Tools\dnSpy`、`C:\Tools\dnspy-mcp-edit-tests` 和配置的 ArtifactRoot；原样本不得覆盖。

## 执行动作

1. 记录本次插件 DLL SHA-256，部署后再次读取 VM 目标 DLL SHA-256，二者必须一致。停止 dnSpy 后才替换 DLL；只清理 dnSpy 自动生成的 `dnSpy-mef-info.bin` 组合缓存，它可由 dnSpy 重建。
...
4. x86 完成后清理所有测试 debuggee、断点、传输会话，恢复 x64 dnSpy 与原远程监听配置。
