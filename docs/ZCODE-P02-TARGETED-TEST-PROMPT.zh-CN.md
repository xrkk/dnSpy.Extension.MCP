...
- 使用仓库 `tests/edit/p02_vm_driver.py` 及 P02 宿主编排执行；只操作
  `C:\Tools\dnSpy`、`C:\Tools\dnspy-mcp-edit-tests` 与配置的 ArtifactRoot。
...
3. x86 结束后清理测试进程、临时验证文件、断点和会话，恢复 x64 dnSpy 与原监听配置。
