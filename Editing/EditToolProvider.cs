...
internal sealed class EditToolProvider : IMcpToolProvider, IDisposable {
	...
	static string Description(string name) => name switch {
		...
		"edit_resource_import" => "Read embedded, linked or Win32 resource bytes from an allowed VM sample file path server-side and stage the corresponding resource operation with an inline payload — large payloads never ride the MCP request body; limits stay unchanged (OUT-008).",
		"edit_resource_export" => "Atomically write committed embedded or Win32 resource bytes below ArtifactRoot and return the handle-derived file identity (path, file_id, length, SHA-256) (OUT-008).",
